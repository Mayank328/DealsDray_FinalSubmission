...
const Record = (props) => (
  <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
    ...
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      <img src={`data:image/png;base64,${props.record.f_Image}`}></img>   
    </td>
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      {props.record.f_Email}
    </td>
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      {props.record.f_Name}
    </td>
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      {props.record.f_Mobile}
    </td>
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      {props.record.f_Designation}
    </td>
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      {props.record.f_Gender}
    </td>
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      {props.record.f_Course}
    </td>
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      {props.record.f_Createddate}
    </td>
    <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
      <div className="flex gap-2">
        <Link
          className="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-slate-100 h-9 rounded-md px-3"
          to={`/edit/${props.record.f_id}`}
        >
          Edit
        </Link>
        <button
          ...
          type="button"
          onClick={() => {
            props.deleteRecord(props.record.f_id);
          }}
        >
          Delete
        </button>
...
Record.propTypes = {
  record: PropTypes.shape({
    ...
    f_id:PropTypes.string.isRequired,

    f_Image:PropTypes.string.isRequired,

    f_Name:PropTypes.string.isRequired,

    f_Email:PropTypes.string.isRequired,

    f_Mobile:PropTypes.string.isRequired,

    f_Designation:PropTypes.string.isRequired,

    f_Gender:PropTypes.string.isRequired,

    f_Course:PropTypes.string.isRequired,

    f_Createddate:PropTypes.string.isRequired,
